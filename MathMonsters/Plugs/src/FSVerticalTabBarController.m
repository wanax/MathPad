...
#define DEFAULT_TAB_BAR_WIDTH 100.0
#define DEFAULT_TOP_IMGHEIGHT 55.0
...
- (FSVerticalTabBar *)tabBar
{
    if (_tabBar == nil)
    {
        _tabBar = [[FSVerticalTabBar alloc] initWithFrame:CGRectMake(0, DEFAULT_TOP_IMGHEIGHT, 100, 100)];
...
- (void)setSelectedIndex:(NSUInteger)selectedIndex
{
    if (selectedIndex != _selectedIndex && selectedIndex < [self.viewControllers count])
    {
        ...
        selectedViewController.view.frame = CGRectMake(self.tabBarWidth,
                                                       DEFAULT_TOP_IMGHEIGHT,
                                                       self.view.bounds.size.width-self.tabBarWidth,
                                                       self.view.bounds.size.height-DEFAULT_TOP_IMGHEIGHT);
...
- (void)_performInitialization
{
    self.tabBarWidth = DEFAULT_TAB_BAR_WIDTH;
...
- (void)loadView
{
    ...
    // create tab bar
    self.tabBar.frame = CGRectMake(0, DEFAULT_TOP_IMGHEIGHT, self.tabBarWidth, layoutContainerView.bounds.size.height);
